const WhyUsItems = ({ image, title, description, isReverse }) => {
  return (
      <div
      className={`w-full whyUs sm:w-1/2whyUs flex flex-col ${
        isReverse ? "md:flex-row-reverse" : "md:flex-row"
      } items-center justify-between mb-16 rounded-3xl`}
    >
      ...
      <div className="flex-1 text-left md:text-center  hover:opacity-100">
        <div className="mx-8">
          ...
          <p className="text-base leading-relaxed text-gray-600 pb-4">{description}</p>
        </div>
      </div>
    </div>
